...
def xml_from_excel3(excel_file):
	...
	root = generateXMLHeader("QC3_GAS_LEAK_CALIB","GEM Chamber QC3 Gas Leak Calib Data",str(location) + " GEM QC3 Gas Calib Data",Run,Start,Stop,comment,location,user)
	...
	for row in range(1,sh.nrows):
			...
			generateXMLData3(dataSet,str(Date),str(seconds),str(pre), str(ambp), str(temp), str(increment_hr), str(temp_k))
	...
	File=sys.argv[9]
	#Comments=sys.argv[10]
	root = generateXMLHeader("QC3_GAS_LEAK_CALIB_SUMRY","GEM Chamber QC3 Gas Leak Calib Summary",str(location) + " GEM QC3 Gas Calib Data",Run,Start,Stop,comment, location,user)
	dataSet = generateDataSet(root,Comments,"1","GEM Chamber",chamber)
