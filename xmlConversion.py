...
def generateXMLDataQCResult(dataSetTag,qc_testText,qc_resultText,elog_linkText, commentsText):
    data = SubElement(dataSetTag, 'DATA')
    qc_test = SubElement(data, 'QC_TEST')
    qc_test.text = qc_testText
    qc_result = SubElement(data, 'QC_RESULT')
    qc_result.text = qc_resultText
    elog_link = SubElement(data, 'ELOG_LINK')
    elog_link.text = elog_linkText
